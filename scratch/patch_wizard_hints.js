const fs = require('fs');
let s = fs.readFileSync('app/assets/wizard.js', 'utf8');

const getStepHintBoxCode = `  function fillStepInput(inputId, val, stateKey) {
...
if (s.includes('  function skipExperienceFresh() {')) {
  s = s.replace('  function skipExperienceFresh() {', getStepHintBoxCode);
  console.log('Injected getStepHintBox and fillStepInput cleanly before skipExperienceFresh');
} else {
  console.log('skipExperienceFresh not found!');
}

// Update case 'education':
const oldEdu = `      case 'education':
        html = \`
...
if (s.includes(oldEdu)) { s = s.replace(oldEdu, newEdu); console.log('Updated education step'); }

// Update case 'experience':
const oldExp = `      case 'experience':
        html = \`
...
if (s.includes(oldExp)) { s = s.replace(oldExp, newExp); console.log('Updated experience step'); }

// Update case 'projects':
const oldProj = `      case 'projects':
        html = \`
...
if (s.includes(oldProj)) { s = s.replace(oldProj, newProj); console.log('Updated projects step'); }

// Export fillStepInput
...
console.log('Wizard hints patch completed successfully.');
